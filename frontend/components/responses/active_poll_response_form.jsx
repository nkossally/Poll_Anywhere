...
class ActivePollResponseForm extends React.Component {
  ...
  render() {
    ...
    let choiceButtons;
    let deleteButton = this.props.choice_id ? (
      <button onClick={this.handleDelete}>Clear last response</button>
    ) : (
      ""
    );
    ...
    if (Object.keys(poll).length !== 0) {
      if (this.props.choices) {
        choiceButtons = this.props.choices.map((choice, idx) => {
          let className =
            this.props.choice_id === choice.id
              ? "response-chosen"
              : "response-choice";
          return (
            <div key={idx}>
              <button
                disabled={this.state.choice_id}
                className={className}
                onClick={this.update(choice.id)}
                value={choice.id}
              >
                {" "}
                {choice.body}{" "}
    ...
    return (
      <ul className="response-blue-box">
        <li className="response-question">{poll.body}</li>
        <li className="response-choice-container">{choiceButtons}</li>
